...
| `quality` | number (1-100) | Image quality (coerced to closest in `images.qualities`) |
| `loading` | `'lazy'` \| `'eager'` | Loading strategy |
| `unoptimized` | boolean | Skip optimization |
| `loader` | function | Custom image loader |

### v16 Image Config Defaults

```ts
// next.config.ts -- v16 defaults (no changes needed unless overriding)
images: {
  minimumCacheTTL: 14400,           // 4 hours (was 60s in v15)
  imageSizes: [32, 48, 64, 96, 128, 256, 384], // 16 removed
  qualities: [75],                  // Was [1..100] -- quality coerced to closest
  dangerouslyAllowLocalIP: false,   // NEW: blocks local IP by default
  maximumRedirects: 3,              // Was unlimited
}
```

### Local Images with Query Strings (v16 Breaking)

Requires `images.localPatterns.search` config:
...
### Generated OG Images (v16 -- async params and id)
...
export async function generateImageMetadata({ params }) {
  const { slug } = params; // synchronous in generateImageMetadata
  return [{ id: '1' }, { id: '2' }];
}

export default async function Image({ params, id }) {
  const { slug } = await params; // v16: async
  const imageId = await id;       // v16: Promise<string>
  const post = await getPost(slug);
...
// Multi-sitemap with async id
export async function generateSitemaps() {
  return [{ id: 0 }, { id: 1 }, { id: 2 }];
}

export default async function sitemap({ id }) {
  const resolvedId = await id;             // v16: Promise<string>
