---
description: 啟動 Manual Loop（一律 ON，覆蓋 ZENBU_HOOKS_ENABLED env）——持續呼叫 acceptance-evaluator 直到 PASS 或達 max-iterations
...
4. **State file 優先（一律 ON 語意）**——寫入 `status: enabled` 的 state file 永遠覆蓋 `ZENBU_HOOKS_ENABLED` env。若先前曾 `/zenbu-powers:zenbu-loop-cancel`（state file 為 `status: disabled`），本指令會 explicit 覆寫為 enabled，重新啟用驗收 loop。
