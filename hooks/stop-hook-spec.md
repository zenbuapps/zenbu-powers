...
>
> **H2 重構後關鍵差異**：reason 內含 `[ZENBU_LOOP_DISPATCH]` token，主窗口 reflex 第 11 條會據此**自動派 acceptance-evaluator sub-agent** 完成驗收。Stop hook 本身不再 spawn agent（先前 agent type 的內嵌 prompt 已移除），純 shell 完成 state machine 的 deterministic 部分。
