# Stop Hook Shell 邏輯規範（zenbu-powers，已封存）

> **⚠️ 已封存**：`hooks/hooks.json` 不再註冊 Stop hook，`hooks/stop-hook` 腳本檔保留但不會被觸發。本檔保留供未來驗收機制重新設計時參考。

歷史說明：由 `hooks/hooks.json` 的 Stop hook 在每次 Claude Code 嘗試結束 session 時觸發 polyglot 腳本 `hooks/stop-hook` 執行。本檔是該 shell 腳本的邏輯說明書。
