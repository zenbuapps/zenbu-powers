...
> **本 agent 為 opt-in**：僅在用戶顯式喚醒時上場。不在自動開發流程中——`@zenbu-powers:nestjs-master` 完成後**不會**自動派本 agent，自動驗收統一由 Stop hook → `@zenbu-powers:acceptance-evaluator` 把關。
