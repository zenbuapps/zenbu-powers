...
**v3.15.0** — **Stop hook 移除**，自動驗收 loop / Auto Loop / Manual Loop 全數退場。改採「主窗口完成 → 直接交付用戶」流程；用戶後續可顯式喚醒 `@acceptance-evaluator` 做一次性驗收。SessionStart 注心法、UserPromptSubmit 注 reflex（9 條交接規則，原 #10 批次 fetch / #11 Stop hook dispatch 一併移除）、AIBDD 條件式注入仍保留；AIBDD 三語言整合測試（TypeScript / PHP / C#）完整覆蓋。
...
Orchestrator（Claude 主窗口）
    ...
    │        ├── acceptance-evaluator  ← 驗收標準對齊（v3.15.0 起 opt-in，用戶顯式喚醒）
...
**兩層 Hook 架構**讓「協調紀律」在第一輪 prompt 之前就到位（v3.15.0 起移除原第三層 Stop hook）：
...
> **v3.15.0 變更**：原第 3 層 Stop hook 已退場（每對話即驗收造成延遲過長），改採「主窗口完成 → 直接交付用戶」流程；驗收改為 opt-in，由用戶顯式喚醒 `@zenbu-powers:acceptance-evaluator`。
...
flowchart TD
    ...
    subgraph 一次性驗收[opt-in 驗收 v3.15.0]
        Z -.-> AE[用戶顯式喚醒<br>@acceptance-evaluator]
        AE -.-> AR[驗收報告<br>PASS / FAIL + 缺陷清單]
    end
...
設定後重啟 session 即可。未啟用時，agents / skills 仍可手動呼叫，但 SessionStart / UserPromptSubmit hook 不會注入心法與 reflex（v3.15.0 起 Stop hook 已移除，無自動驗收 loop）。
...
| `wordpress-reviewer` | WP 程式碼審查（**opt-in**：用戶顯式喚醒才上場；不在自動流程中） |
...
| `acceptance-evaluator` | 驗收標準對齊審查，純做 user-intent alignment、需求覆蓋度、邊界完整性、off-topic 偵測（**opt-in**：v3.15.0 起改由用戶顯式喚醒） |
...
zenbu-powers 目前註冊 3 個 slash command：
...
> **v3.15.0 變更**：`/zenbu-loop`、`/zenbu-loop-status`、`/zenbu-loop-cancel` 三個 command 對應的 Stop hook driven loop 已退場。檔案保留在 `commands/zenbu-loop*.md` 但 state file 機制無 trigger source，呼叫不會啟動 evaluator loop。

---

## Hook 兩層架構詳解（v3.15.0）

> **v3.15.0 變更**：原 Stop hook 已從 `hooks/hooks.json` 移除，自動驗收 loop / Auto Loop / Manual Loop 全數退場。改為「主窗口完成 → 直接交付用戶」流程，由用戶決定何時做一次性驗收。本章節僅描述目前保留的 SessionStart 與 UserPromptSubmit 兩層 hook。
...
1. **Reflex Dictionary（無條件）** — `hooks/reflex-dictionary.txt` 的 9 條協調紀律：
   - ① 命名規範 ② orchestrator 角色 ③ 自主決策 ④ 鏈式委派 ⑤ 驗收紀律（opt-in） ⑥ 第一性原理 default-on ⑦ 不中途停下 ⑧ WHAT vs HOW ⑨ Skill 優先順序
...
### 3. Stop Hook — 已於 v3.15.0 移除

原 v3.13 引入的 Stop hook driven acceptance loop（Auto / Manual 雙模式）已整段退場。

**移除原因**：每次 turn 都跑 evaluator 驗收造成對話延遲過長；改採「主窗口完成 → 直接交付用戶」的快速流程，由用戶在完成一輪完整開發後手動觸發一次性驗收與測試。

**保留資源**（檔案仍存在但已不再被 hook 觸發，未來驗收機制重新設計時可復用）：
- `hooks/stop-hook` shell 腳本
- `hooks/stop-hook-spec.md`、`hooks/zenbu-loop-batch-protocol.md` 規範文件
- `commands/zenbu-loop*.md` slash command（**已停用**，state file 機制無 trigger source）
- `agents/acceptance-evaluator.agent.md`（**opt-in**：用戶顯式喚醒 `@zenbu-powers:acceptance-evaluator` 才上場）

**手動驗收用法**：

```text
@zenbu-powers:acceptance-evaluator 對本輪交付物進行對齊驗收
```

evaluator 會跑「零假設驗收前置鐵律」（反向訊號優先掃描、覆蓋度、邊界完整性、off-topic 偵測）後回報 PASS / FAIL + 缺陷清單。
...
Sub-agent 回報後，主窗口讀取「Hand-off / Next Agent」標示自動 dispatch 下一位，不停下問用戶。`*-reviewer` / `acceptance-evaluator` agents **不在**自動鏈中（opt-in：用戶顯式喚醒才上場）。預設純 sub-agent 鏈式委派，不開 Teams / Worktree。
...
### Stop hook 與自動驗收 loop 退場（v3.15.0）

v3.13 引入的「Stop hook 驅動 acceptance loop（Auto / Manual 雙模式）」已於 v3.15.0 整段移除。每對話即驗收造成延遲過長，改採「主窗口完成 → 直接交付用戶」流程。

- **Orchestrator 行為**：sub-agent 回報後不主動 dispatch `@acceptance-evaluator`，直接彙整給用戶
- **驗收觸發點**：用戶在完成一輪完整開發後，顯式喚醒 `@zenbu-powers:acceptance-evaluator` 做一次性驗收
- **窄門例外**：用戶 prompt 含「驗收 / 評估 / final check」等明確關鍵詞，或多 agent 整合 conflict 想做 sanity check 時，orchestrator 可中途顯式派一次 evaluator
...
zenbu-powers/
├── .claude-plugin/
│   ├── plugin.json            # Claude Code Plugin 主設定（version=3.15.0）
│   └── marketplace.json       # Marketplace 上架設定
├── hooks/                     # 兩層 Hook 架構（v3.15.0 移除 Stop hook 註冊）
...
│   ├── stop-hook              # （保留檔案但已停用：v3.15.0 hooks.json 不再註冊）
│   ├── stop-hook-spec.md      # Stop hook 完整邏輯規範（封存）
│   └── zenbu-loop-batch-protocol.md  # FAIL 缺陷分批協定 v2（封存）
├── agents/                    # 23 個 Agent 定義檔
│   ├── wordpress-master.agent.md
│   ├── nestjs-master.agent.md
│   ├── acceptance-evaluator.agent.md  # opt-in：用戶顯式喚醒才上場
│   └── ...
...
│   └── ...
├── commands/                  # 3 個現役 Slash Commands + 3 封存
│   ├── ide-on.md / ide-off.md / ide-status.md  # IDE MCP 切換（現役）
│   └── zenbu-loop*.md         # （封存：Stop hook 退場後 trigger source 無效）
├── scripts/                   # 輔助 Node.js 腳本
│   ├── ide-toggle.mjs         # IDE MCP 切換
│   ├── zenbu-loop-*.mjs       # （封存：對應 commands 已停用）
│   └── release.sh             # 發版腳本
├── prompts/                   # Prompt 範本
...
5. **完成 → 直接交付用戶**（v3.15.0 起 Stop hook 已退場，無自動驗收 loop）
6. **（Opt-in）`@acceptance-evaluator`** — 用戶完整一輪開發後顯式喚醒做一次性對齊驗收
7. **（Opt-in）`@wordpress-reviewer` + `@security-reviewer`** — 用戶顯式喚醒做深度 code review / 安全審查（涉及 auth / payment / REST API 等敏感領域建議補派）
8. **`/git-commit`** — 產生符合慣例的 Commit Message
...
3. **完成 → 直接交付用戶**（v3.15.0 起 Stop hook 已退場，無自動驗收 loop）
4. **（Opt-in）`@acceptance-evaluator` + `@nestjs-reviewer`** — 用戶顯式喚醒做對齊驗收與 DI / Guards / DTO 驗證深度審查
...
完整一輪開發完成後，顯式喚醒 evaluator 做一次性對齊驗收：

```
@zenbu-powers:acceptance-evaluator 對本輪所有交付物進行對齊驗收
```

evaluator 跑「零假設驗收前置鐵律」（反向訊號掃描、覆蓋度、邊界完整性、off-topic 偵測）後回報 PASS / FAIL + 缺陷清單。FAIL 則用戶再決定要不要繼續派 master 修。

> **與 v3.14 之前的差異**：`/zenbu-loop` Manual Loop（Stop hook + state file 機制）已於 v3.15.0 退場——改由用戶在「完整一輪開發後」主動觸發一次性驗收，避免「每對話即驗收」造成的延遲累積。
...
> v3.15.0 起 zenbu-powers 已不再註冊 Stop hook，因此 Stop hook 衝突問題已自動消除（superpowers 若有自己的 Stop hook 邏輯仍會運作）。
...
### Q10：為什麼 v3.15.0 移除自動驗收 Loop？

**答：** 原 v3.13 設計每對話 Stop 時都自動派 `@acceptance-evaluator` 跑驗收方法論，理論上能擋住「主窗口偷懶」的交付，但實務上：

- **每次 turn 都 +1 個 evaluator round-trip** —— 對話延遲明顯拉長
- **小修改 / 探索性對話被過度驗收** —— evaluator 設計給「最終交付」級別任務，套在小改動上 ROI 低
- **FAIL → 餵缺陷 → 主窗口再 stop → 又 FAIL** 可能形成耗時迴圈，10 輪上限保護仍嫌貴

**v3.15.0 新流程**：

- 主窗口完成 → 直接交付用戶
- 用戶在「完成一輪完整開發」後顯式喚醒 `@zenbu-powers:acceptance-evaluator` 跑一次性對齊驗收
- 對小改動 / 探索性對話 → 直接看結果 / git diff，免驗收成本

未來若驗收機制重新設計（觸發條件可控、成本可調），相關檔案（`hooks/stop-hook`、`hooks/stop-hook-spec.md`、`commands/zenbu-loop*.md`）仍保留供復用。
...
`systematic-debugging`、`finishing-branch`、`tdd-workflow` 的 verification-gate 設計，以及 SessionStart hook 架構（polyglot wrapper、多 IDE 平台輸出適配），方法論承襲自 [obra/superpowers](https://github.com/obra/superpowers)，並在地化為 zenbu org 的技術棧場景。zenbu-powers 在此基礎上擴充自主決策授權、零假設驗收鐵律、acceptance-evaluator 多階段驗收紀律（v3.15.0 起改為 opt-in 一次性驗收），以及 AIBDD 三語言（TypeScript / PHP / C#）整合測試套件。
