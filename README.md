...
> **與 opt-in reviewer loop 的關係**：master 完成後**不**自動派 reviewer，最終驗收統一走 Stop hook → acceptance-evaluator（Auto 10 / Manual 自訂輪數）。若用戶顯式喚醒 `*-reviewer` 並進入 reviewer ↔ master 修復迴圈，仍適用「最多 3 輪」上限——這是 opt-in 機制，與 Stop hook 上限是不同層次。
...
Sub-agent 回報後，主窗口讀取「Hand-off / Next Agent」標示自動 dispatch 下一位，不停下問用戶。`*-reviewer` agents **不在**自動鏈中（opt-in：用戶顯式喚醒才上場），自動驗收統一由 Stop hook → `acceptance-evaluator` 把關。若用戶啟動 opt-in reviewer loop，orchestrator 扛中繼。預設純 sub-agent 鏈式委派，不開 Teams / Worktree。
...
`*-reviewer` agents 預設**不在**自動開發流程中——`*-master` 完成後直接交給主窗口，自動驗收由 Stop hook → `acceptance-evaluator` 對齊用戶意圖把關。

若用戶顯式喚醒 reviewer 並要求進入「reviewer ↔ master」修復迴圈，仍適用「**最多 3 輪**」上限，第 3 輪未過則升級用戶裁決。涉及 auth / payment / external-api 等敏感領域，acceptance-evaluator 的 PASS 報告會主動建議補派 `@security-reviewer`。
...
5. **嘗試 stop** → Stop hook 自動派 **`@acceptance-evaluator`** 驗收用戶原始需求覆蓋度與邊界完整性，PASS 才放行
6. **（Opt-in）`@wordpress-reviewer` + `@security-reviewer`** — 用戶顯式喚醒做深度 code review / 安全審查（涉及 auth / payment / REST API 等敏感領域，evaluator PASS 報告會主動建議補派）
...
3. **嘗試 stop** → Stop hook 自動派 **`@acceptance-evaluator`** 驗收
4. **（Opt-in）`@nestjs-reviewer`** — 用戶顯式喚醒審查 DI、Guards、DTO 驗證等程式品質深度
