...
> **預設不啟用（僅指 SessionStart 心法注入）。** 此 hook 注入 context 是個人偏好，需手動啟用。
>
> ⚠️ **Stop hook 行為相反**：驗收 loop 是品質底線，**預設啟用**，`ZENBU_HOOKS_ENABLED=0` 才關閉。詳見「Stop Hook & Loop 模式」段。三個 hook 共用同一個 env 變數但語意分流：
>
> | Hook | 預設 | 啟用條件 |
> |---|---|---|
> | SessionStart（心法注入） | 不啟用 | `ZENBU_HOOKS_ENABLED=1` |
> | UserPromptSubmit（reflex / AIBDD） | 不啟用 | `ZENBU_HOOKS_ENABLED=1` |
> | Stop（驗收 loop） | **啟用** | `ZENBU_HOOKS_ENABLED=0` 顯式關閉 |
...
1. **Reflex Dictionary（無條件）** — `hooks/reflex-dictionary.txt` 的 11 條協調紀律：命名規範、orchestrator 角色、自主決策、鏈式委派、驗收紀律、第一性原理、不中途停下、WHAT vs HOW、Skill 優先順序、批次驗收 fetch、Stop hook dispatch fetch
2. **AIBDD 模式（條件式）** — `hooks/aibdd-mode-prompt.txt` 的 dispatch 路由表，僅當 prompt 含 `AIBDD` / `BDD` / `TDD` 觸發詞時注入

> **預設不啟用。** 與 SessionStart hook 共用 `ZENBU_HOOKS_ENABLED=1` env，啟用方式同上；不影響 Stop hook 的預設啟用。
...
| 方式 | 啟用條件 | max | 任務來源 | 適合 |
|---|---|---|---|---|
| **Auto Loop** | **預設啟用**；`ZENBU_HOOKS_ENABLED=0` 顯式關閉 | 10（hard-coded） | transcript 第一個 user message | 全域品質把關（每次 session 都受 evaluator 監督）|
| **Manual Loop** | `/zenbu-loop <task>` 寫 state file | `--max <n>`（預設 10，0 = unlimited） | state file body 的 task 全文（最可靠）| 明確 task 範圍 / 沒設 env 也想用 |

顯式設 `ZENBU_HOOKS_ENABLED=0` 才關閉 Stop hook。

啟用優先順序：state file 存在 → Manual；否則檢查 env（預設啟用，"0" 才關）→ Auto；ZENBU_HOOKS_ENABLED=0 → 不觸發。
...
> 💡 **為什麼沒有 Playwright MCP？** zenbu-powers 的瀏覽器自動化統一改用 **`playwright-cli` SKILL**（直接呼叫 CLI），不走 MCP server。這樣做的好處：啟動快、無需常駐 process、跨平台穩定、debug 時可直接在終端重現指令。`browser-tester`、`uiux-reviewer`、`test-creator`、`markdown-creator` 等 agent 皆已改用此模式。
