...
**兩層 Hook 架構**：SessionStart 注心法、UserPromptSubmit 注 reflex（9 條交接規則）、AIBDD 條件式注入。驗收為 opt-in——主窗口完成後直接交付用戶，用戶可顯式喚醒 `@acceptance-evaluator` 做一次性驗收。AIBDD 三語言整合測試（TypeScript / PHP / C#）完整覆蓋。
...
Orchestrator（Claude 主窗口）
    ...
    │        ├── acceptance-evaluator  ← 驗收標準對齊（opt-in：用戶顯式喚醒）
...
驗收為 opt-in：主窗口完成後直接交付用戶，由用戶顯式喚醒 `@zenbu-powers:acceptance-evaluator` 做一次性驗收。
...
| `acceptance-evaluator` | 驗收標準對齊審查，純做 user-intent alignment、需求覆蓋度、邊界完整性、off-topic 偵測（**opt-in**：用戶顯式喚醒才上場） |
...
> **註**：原 `/zenbu-loop`、`/zenbu-loop-status`、`/zenbu-loop-cancel` 三個 command 對應的 Stop hook driven loop 已退場，相關檔案已刪除。
...
主窗口完成 → 直接交付用戶；驗收由用戶在完成一輪完整開發後手動觸發。

**保留資源**（檔案仍存在但已不再被 hook 觸發，未來驗收機制重新設計時可復用）：
- `hooks/stop-hook` shell 腳本（已封存）
- `hooks/stop-hook-spec.md`、`hooks/zenbu-loop-batch-protocol.md` 規範文件（已封存）
...
### Q10：為什麼驗收是 opt-in 而非每對話自動跑？

**答：** 早期設計過每對話 Stop 時自動派 `@acceptance-evaluator` 跑驗收方法論，理論上能擋住「主窗口偷懶」的交付，但實務上：
...
未來若驗收機制重新設計（觸發條件可控、成本可調），相關檔案（`hooks/stop-hook`、`hooks/stop-hook-spec.md`）仍保留供復用。
...
`systematic-debugging`、`finishing-branch`、`tdd-workflow` 的 verification-gate 設計，以及 SessionStart hook 架構（polyglot wrapper、多 IDE 平台輸出適配），方法論承襲自 [obra/superpowers](https://github.com/obra/superpowers)，並在地化為 zenbu org 的技術棧場景。zenbu-powers 在此基礎上擴充自主決策授權、零假設驗收鐵律、acceptance-evaluator opt-in 一次性對齊驗收，以及 AIBDD 三語言（TypeScript / PHP / C#）整合測試套件。
