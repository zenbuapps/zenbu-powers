...
description: >
  ...
  判斷 tailwindcss pin 的 major 版本——pin ^3.x 載入 references/v3/REFERENCE.md；
  pin ^4.x 載入 references/v4/REFERENCE.md。
...
2. 比對 `dependencies` / `devDependencies` 中 `tailwindcss` pin 的 major 版本：
   - **`^3.x`**（或 `~3.x` / `3.x.x`）→ Read `references/v3/REFERENCE.md`
...
詳細 API 與 v3 ↔ v4 完整對照：見 `references/v3/REFERENCE.md` 與 `references/v4/REFERENCE.md`。
...
- 同步交付：`references/v3/REFERENCE.md`、`references/v3/v3-to-v4-migration.md`、`references/v4/REFERENCE.md`、`references/v4/{breaking-changes,directives-and-theme,utilities-examples,variants}.md`。
