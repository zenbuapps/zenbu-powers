# Output Schema：zenbu-loop Batch Protocol v2

> 本檔抽自 `hooks/zenbu-loop-batch-protocol.md` Section 1。
...
當 acceptance-evaluator 在 zenbu-loop 體系內被 dispatch（無論是否明示 zenbu-loop session），**報告結尾必須輸出**符合本 schema 的 fenced JSON code block。stop-hook 會從 transcript 抽 last fenced JSON 以判定 verdict 並組裝下輪 reason。
