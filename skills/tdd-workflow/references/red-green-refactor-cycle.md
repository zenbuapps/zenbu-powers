...
> **安全敏感任務提示**：涉及 auth / payment / external-api / SQL / nonce 等敏感領域時，應在報告中明示建議用戶補派 `@zenbu-powers:security-reviewer`。
...
| 用戶顯式喚醒 reviewer 後 FAIL | 用戶決定是否派 master 修；若進入 reviewer ↔ master 修復迴圈，最多 3 輪 |
