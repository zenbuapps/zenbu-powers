...
description: >
  ...
  當 aibdd-auto-red 或 aibdd-auto-step-template 在處理 Given/When/Then 步驟時觸發。
---
...
>
> **非 user-invocable**：由 `aibdd-auto-red`、`aibdd-auto-step-template` 等上游 skill 載入；使用者不直接呼叫。
...
- 本階段**未刪除**舊 18 個 `aibdd.auto.{lang}.it.handlers.*` skill，**未修改**任何下游引用（`aibdd-auto-red`、`test-creator.agent.md`、其他 step-template / refactor / red 等）。
