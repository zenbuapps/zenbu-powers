...
Fallback for unmatched slots during full-page reload (hard navigation).

- **Soft navigation** (client-side `<Link>`): maintains active state of unmatched slots.
- **Hard navigation** (refresh/direct URL): renders `default.tsx` for unmatched slots, or 404 if none.

```tsx
// app/@analytics/default.tsx
export default function Default() {
  return null; // Render nothing when no matching route
}
```
...
export default function Layout({ auth }: { auth: React.ReactNode }) {
  ...
  // loginSegment === 'login' when at /login
}
```
...
The most common use case: opening content as a modal on soft navigation, but as a full
page on hard navigation (direct URL or refresh).
...
app/
  ...
  page.tsx
  @auth/
    default.tsx           <- returns null (modal hidden)
...
Note: In recent Next.js versions, `reset` may be named `unstable_retry`.
...
- Unhandled promise rejections outside of `startTransition` (use error reporting).
