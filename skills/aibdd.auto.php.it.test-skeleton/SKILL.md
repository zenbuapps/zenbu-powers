...
description: >
  ...
  本 skill 不再維護。請改用：
    ...
    - php test-skeleton 範例：references/test-skeleton/php.md
deprecated: true
---

# [DEPRECATED] aibdd.auto.php.it.test-skeleton
...
1. 主 skill：`zenbu-powers:aibdd-auto-tdd`（stage=test-skeleton, lang=php）
2. 流程總圖：`references/pipeline-overview.md`
3. php test-skeleton 程式碼範例：`references/test-skeleton/php.md`

詳見 `docs/refactor-3-audit.md` 與新主 skill 的 Hand-off 段。

## 何時刪除此 stub

3 個版本後（或 6 個月後）若 grep 全工作目錄無 `aibdd.auto.php.it.test-skeleton` 命中，可移除整個 skill 目錄。
