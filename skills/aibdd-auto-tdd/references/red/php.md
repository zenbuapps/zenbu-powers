# red — PHP WordPress Integration Test

> 主 SKILL.md 已涵蓋：trigger 辨識、紅燈定義（環境正常 + Value Difference）、共用三步驟流程框架、共用 R 規則。本檔僅提供 PHP 特化內容。

PHP IT Stage 2：對單一 .feature 的測試骨架執行完整紅燈流程：

1. 建立 `IntegrationTestCase` 基類（若不存在）
...
接手 `references/test-skeleton/php.md` 階段產出的 PHPUnit 測試骨架（含 `markTestIncomplete` 與 Gherkin TODO 註解），完成 IntegrationTestCase 基類、Stub class、Test method 三大實作。
...
由 `references/control-flow/php.md` 流程批次呼叫，接收 **feature file 絕對路徑** 作為參數：

```
aibdd-auto-tdd (stage=red, lang=php, feature=specs/features/01-lesson-progress.feature)
```
...
完成後告知使用者「紅燈完成，可進入 Stage 3 綠燈階段（依 `references/green/php.md`）」。
