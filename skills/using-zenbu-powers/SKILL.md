...
description: 在做出任何回應（包含釐清提問）之前先檢查是否適用——若適用，載入此 skill 以建立 orchestrator 心法、鏈式委派紀律、acceptance evaluation（opt-in）與全域一致性原則。
---
...
**`*-reviewer` 與 `acceptance-evaluator` 不在自動鏈中**——它們是 opt-in（僅在用戶顯式喚醒時上場）。`*-master` 完成後直接交給主窗口、主窗口彙整交付用戶；驗收由用戶自行在「完整一輪開發後」喚醒 `@zenbu-powers:acceptance-evaluator` 跑一次性對齊驗收。若用戶顯式發起 reviewer ↔ master 修復迴圈，主窗口仍扛中繼（最多 3 輪）。
...
## 驗收評估（Acceptance Evaluation — opt-in）
...
完整 dispatch 規格、reviewer 與 evaluator 的職責邊界、驗收策略詳見 `references/acceptance-loop.md`。
