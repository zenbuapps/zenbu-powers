...
**`*-reviewer` agents 不在自動鏈中**——它們是 opt-in（僅在用戶顯式喚醒時上場做深度 code review）。`*-master` 完成後直接交給主窗口，最終驗收統一由 Stop hook → `@zenbu-powers:acceptance-evaluator` 對齊用戶意圖把關。若用戶顯式發起 reviewer ↔ master 修復迴圈，主窗口仍扛中繼（最多 3 輪）。
