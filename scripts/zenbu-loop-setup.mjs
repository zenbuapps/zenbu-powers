...
對比 Auto Loop（預設啟用，max=10；ZENBU_HOOKS_ENABLED=0 顯式關閉）：
  Manual 適合：明確 task 範圍 / 需要自訂 max / 任務有明文邊界
  Auto 適合：日常 session，所有結尾都受 evaluator 把關（10 輪保護）
...
const envNote = envEnabled
	? '（Auto Loop 預設啟用——Manual Loop 設定優先生效）'
	: '（ZENBU_HOOKS_ENABLED=0 已顯式關閉 Auto Loop——Manual Loop 仍可獨立運作）';
